...
func (h *RecapCallbackQueryHandler) Handle(c *tgbot.Context) error {
	...
	if !lo.Contains(RecapSelectHourAvailables, data.Hour) {
		return tgbot.NewExceptionError(fmt.Errorf("invalid hour: %d", data.Hour)).WithReply(c.Update.CallbackQuery.Message.ReplyToMessage)
	}
	...
	if err != nil {
		return tgbot.NewExceptionError(err).WithMessage("聊天记录回顾生成失败，请稍后再试！").WithReply(c.Update.CallbackQuery.Message.ReplyToMessage)
	}
	if len(histories) <= 5 {
		return tgbot.NewMessageError(fmt.Sprintf("最近 %d 小时内暂时没有超过 5 条的聊天记录可以生成聊天回顾哦，要再多聊点之后再试试吗？", data.Hour)).WithReply(c.Update.CallbackQuery.Message.ReplyToMessage)
	}
	...
	if summarization == "" {
		return tgbot.NewMessageError("聊天记录回顾生成失败，请稍后再试！").WithReply(c.Update.CallbackQuery.Message.ReplyToMessage)
	}

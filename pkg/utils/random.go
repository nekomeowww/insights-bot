...
const (
	defaultRandBytesLength = 32

	defaultRandomHashStringRandomBytesLength = 64
	maxRandomHashStringLength                = 64
)

// RandBytes 根据给定的长度生成字节，长度默认为 32。
func RandBytes(length ...int) ([]byte, error) {
	b := make([]byte, defaultRandBytesLength)
	if len(length) != 0 {
...
func RandomHashString(length ...int) string {
	b, _ := RandBytes(defaultRandomHashStringRandomBytesLength)

	if len(length) != 0 {
		sliceLength := length[0]
		if sliceLength > maxRandomHashStringLength || sliceLength <= 0 {
			sliceLength = maxRandomHashStringLength
		}

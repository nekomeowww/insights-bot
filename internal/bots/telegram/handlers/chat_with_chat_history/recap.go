...
func (h *Handler) HandleCallbackQuery(c *handler.Context) {
	...
	if !lo.Contains(RecapSelectHourAvailables, data.Hour) {
		...
		errMessage := tgbotapi.NewMessage(c.Update.CallbackQuery.Message.Chat.ID, "出现了一些问题，请稍后再试哦")
		errMessage.ReplyToMessageID = c.Update.CallbackQuery.Message.MessageID
		c.Bot.MustSend(errMessage)
	}

	_, err = c.Bot.Request(tgbotapi.NewDeleteMessage(c.Update.CallbackQuery.Message.Chat.ID, c.Update.CallbackQuery.Message.MessageID))
	if err != nil {
